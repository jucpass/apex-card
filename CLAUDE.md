...
- File uploads (e.g. Country destination images) go through `packages/api` using `SUPABASE_SERVICE_ROLE_KEY` (`packages/api/src/lib/supabaseAdmin.ts`) — never expose the service-role key to Admin Web or mobile; store the Storage object path in Postgres, not a bare public URL.
- Partner management lives in the Partner Details Workspace (`/admin/partners/:id`) — future Partner features (discounts, featured, ratings, analytics) extend that page's Configuration placeholders instead of adding separate Partner pages. Partners start as DRAFT and are never deleted (activate/deactivate only). Partner images: `PartnerMedia` table + public `partner-assets` bucket, max 3 per partner, exactly one cover, ≥1 image required before activation. Categories are many-to-many via `PartnerCategory`; one physical location per Partner is an intentional MVP rule (no `PartnerLocation` table — branches are separate Partners); `googleMapsUrl` is the preferred mobile navigation link; the Partner form prefills the country's `phoneCode` into empty phone/WhatsApp fields (never overwriting typed numbers).
